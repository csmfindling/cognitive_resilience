    ...
    def work(self, sess, saver, train, num_trials=100):
        ...
        while True:
            ...
            if episode_count % 50 == 0 and episode_count != 0:
                ...
                summary = tf.Summary()
                summary.value.add(tag='Perf/reward_train_RNN', simple_value=float(mean_reward))                
                summary.value.add(tag='Perf/reward_train_oracle', simple_value=float(np.mean(self.performance_oracle[-50:])))
                summary.value.add(tag='Test/reward_test_RNN', simple_value=float(self.episode_reward_test[-1]))
                summary.value.add(tag='Test/reward_test_oracle', simple_value=float(self.episode_reward_test_oracle[-1]))
